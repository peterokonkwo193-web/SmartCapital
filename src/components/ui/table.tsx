...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-11 whitespace-nowrap px-3 text-left align-middle text-xs font-semibold uppercase tracking-wide text-muted-foreground first:pl-0 last:pr-0",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn("whitespace-nowrap px-3 py-3.5 align-middle first:pl-0 last:pr-0", className)}
      {...props}
    />
  );
}
